...
variables 
    int[1,5] cmd		/* set1 = 1 set2 = 2 read = 3 dec  = 4 inc  = 5 */
    int[1,2] stored
...
crash locations: Candidate, Leader, Replica, RepCmd;
...
location RepCmd
    on ValueCons<vcCmd>(All,1,cmd) do
      ...
      if(vcCmd.desVal == 4 && stored != 2) { /* increment */
          stored = stored + 1;
          rend(ackCmd[cmd], inc.sID);
      }
      if(vcCmd.desVal == 5&& stored != 1) { /* decrement */
          stored = stored - 1;
