...
//
//import lang.core.Handler;
//import lang.core.HandlerPartitionCons;
//import lang.core.HandlerPredicated;
//import lang.core.HandlerValueCons;
...
//
//public class ExistenceChecker_withExplicitTrailFunction {
//
//	private ProcessSemantics ps;
//	PathFinder pathFinder;
//
//	public ExistenceChecker_withExplicitTrailFunction(ProcessSemantics ps) {
//		this.ps = ps;
...
//		ArrayList<Expression> existenceConjuncts = new ArrayList<Expression>();
//
...
//
//			ArrayList<Expression> bodyDisjuncts = new ArrayList<Expression>();
//			for (Handler handler : handlers) {
...
//					ArrayList<Expression> currentBodyConjuncts = new ArrayList<Expression>();
//
...
//										new ExprConstant(src.getLoc().getName(), TypeLoc.T()));
...
//		ArrayList<Expression> phasesDisjuncts = new ArrayList<Expression>();
...
//
//		// so we do this once.
//		boolean s1IsCore = srcPhase.isCoreState(s1);
//		boolean s2IsCore = srcPhase.isCoreState(s2);
//		ArrayList<Expression> conjuncts = new ArrayList<Expression>();
//		if (s1IsCore && s2IsCore) {
//
//			/**	  
//			*    a. both states are in the core states of the phase. Then, we return the following constraint:
//			* 		  exists(s1, phase.label, null, true, true, false) and 
...
//			*/
//
//			// build the trail from s2 back to a core state.
//			Trail trailToS2 = srcPhase.buildTrailTo(s2);
//			conjuncts.add(exists(s1));
//			conjuncts.add(existsLabelFrom(s1, srcPhase.getLabel()).getFirst());
//			conjuncts.add(exists(trailToS2));
//			conjuncts.add(existsLabelFrom(trailToS2.getInitialState(), srcPhase.getLabel()).getFirst());
//
//		} else if (!s1IsCore && s2IsCore) {
//
//			// as before, build the trail from s1 back to a core state.
//			Trail trailToS1 = srcPhase.buildTrailTo(s1);
//			conjuncts.add(exists(s2));
//			conjuncts.add(existsLabelFrom(s2, srcPhase.getLabel()).getFirst());
//			conjuncts.add(exists(trailToS1));
//			conjuncts.add(existsLabelFrom(trailToS1.getInitialState(), srcPhase.getLabel()).getFirst());
//
//		} else {
//
//			/** 
...
//			LocalState s2) {
//		/**
//		* if the phase is a core singleton, then we use a similar concept to the one up there, distinguishing 
//		*  two possible scenarios: one is core and one is an expansion, and, both are expansions.
//		*/
//
//		// so we do this once.
//		boolean s1IsCore = singeletonPhase.isCoreState(s1);
//		boolean s2IsCore = singeletonPhase.isCoreState(s2);
//
//		ArrayList<Expression> conjuncts = new ArrayList<Expression>();
//
//		if (s1IsCore && s2IsCore) {
//
//			// should never happen.
//			System.err.println("singeleton with two core states? " + singeletonPhase);
//			Thread.dumpStack();
//			System.exit(-1);
//
//		} else if (s1IsCore && !s2IsCore) {
//
//			// build the trail from s2 back to a core state.
//			Trail trailToS2 = singeletonPhase.buildTrailTo(s2);
//			conjuncts.add(exists(s1));
//			conjuncts.add(exists(trailToS2));
//
//		} else if (!s1IsCore && s2IsCore) {
//
//			// as before, build the trail from s1 back to a core state.
//			Trail trailToS1 = singeletonPhase.buildTrailTo(s1);
//			conjuncts.add(exists(s2));
//			conjuncts.add(exists(trailToS1));
//
//		} else {
...
//
//		// so we do this once.
//		boolean s1IsCore = destPhase.isCoreState(s1);
//		boolean s2IsCore = destPhase.isCoreState(s2);
//		ArrayList<Expression> conjuncts = new ArrayList<Expression>();
//
//		if (s1IsCore && s2IsCore) {
//
...
//			 */
//
//			// these ensure the state itself exist, too.
...
//
//			// build the trail from s2 back to a core state.
//			Trail trailToS2 = destPhase.buildTrailTo(s2);
//			conjuncts.add(existsLabelTo(s1, destPhase.getLabel()).getFirst());
//			conjuncts.add(existsLabelTo(trailToS2.getInitialState(), destPhase.getLabel()).getFirst());
//			conjuncts.add(exists(trailToS2));
//
//		} else if (!s1IsCore && s2IsCore) {
//
//			// build the trail from s1 back to a core state.
//			Trail trailToS1 = destPhase.buildTrailTo(s1);
//			conjuncts.add(existsLabelTo(s2, destPhase.getLabel()).getFirst());
//			conjuncts.add(existsLabelTo(trailToS1.getInitialState(), destPhase.getLabel()).getFirst());
//			conjuncts.add(exists(trailToS1));
//
//		} else {
//
...
//			 */
//
//			// build the trail from s1 and s2 back to a core state.
//			Trail trailToS1 = destPhase.buildTrailTo(s1);
//			Trail trailToS2 = destPhase.buildTrailTo(s2);
//
//			conjuncts.add(existsLabelTo(trailToS1.getInitialState(), destPhase.getLabel()).getFirst());
